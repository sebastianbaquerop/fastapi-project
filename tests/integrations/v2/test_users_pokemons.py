...
def test_get_user_value_error(test_client, mock_http_client,general_internal_server_error):
    ...
    with patch('app.api.v2.endpoints.users.UserAndPokemonService.get_users') as mock_service:
        #Arrange
...
def test_post_user_success(test_client,user_and_pokemons_create_dto_mock,
    ...
    mock_response = post_users_and_pokemons_response_success_mock.model_dump(mode='json')
    #Act
...
def test_post_user_with_pokemons_error(test_client,user_and_pokemons_create_dto_mock,
                                 post_error):
...
def test_get_user_by_id_success(test_client, get_user_and_pokemons_info_by_id_response_success_mock):
    """Test to retrieve information of id user with success"""
    #Arrange
    user_id = "1"
    data = get_user_and_pokemons_info_by_id_response_success_mock.model_dump(mode='json')
    status_code = 200
    #Act
    response = test_client.get("/v2/users/"+user_id)
    #Assert
...
def test_put_success(test_client,
                     user_and_pokemons_update_dto_mock,put_users_and_pokemons_response_success_mock):
...
def test_put_user_error_un_processable_content(test_client,
                                               user_and_pokemons_update_dto_mock,general_error):
...
def test_patch_user_success(test_client,
                       patch_user_and_pokemons_patch_dto_mock,patch_users_and_pokemons_response_success_mock):
    ...
    mock_response = patch_users_and_pokemons_response_success_mock.model_dump(mode='json')
    status_code = 200
    #Act
...
def test_patch_user_error_un_processable_content(test_client,
                                                 patch_user_and_pokemons_patch_dto_mock,general_error):
