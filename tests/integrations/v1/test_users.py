...
def test_get_user_value_error(test_client, mock_http_client,general_internal_server_error):
...
def test_get_user_success_empty_register(test_client, general_error):
    ...
    status_code = 404
    #Act
    response = test_client.get("/v1/users/"+user_id)
    #Assert
    assert response.status_code == status_code
    assert response.json() == data

def test_post_user_success(test_client,user_create_dto_mock,
                                 post_users_response_success_mock):
    """Test for creating user successfully"""
    #Arrange
    status_code = 201
    payload = user_create_dto_mock.model_dump(mode='json')
    mock_response = post_users_response_success_mock.model_dump(mode='json')
    #Act
    response = test_client.post("/v1/users", json=payload)
    assert response.status_code == status_code
    assert json.dumps(response.json(), sort_keys=True) == json.dumps(mock_response, sort_keys=True)

def test_post_user_with_pokemons_error(test_client,user_create_dto_mock,
                                 post_error):
...
def test_get_user_by_id_success(test_client, get_user_info_by_id_response_success_mock):
    ...
    #Arrange
    user_id = "1"
    data = get_user_info_by_id_response_success_mock.model_dump(mode='json')
    status_code = 200
    #Act
    response = test_client.get("/v1/users/"+user_id)
    #Assert
...
def test_put_success(test_client,
                     user_update_dto_mock,put_users_response_success_mock):
    """Test to update complete info user with success"""
    #Arrange
    user_id = "1"
    payload = user_update_dto_mock.model_dump(mode='json')
    mock_response = put_users_response_success_mock.model_dump(mode='json')
    status_code = 200
    #Act
    response = test_client.put("/v1/users/"+user_id, json=payload)
...
def test_put_user_error_un_processable_content(test_client,
                                               user_update_dto_mock,general_error):
...
def test_patch_user_success(test_client,
                       patch_user_patch_dto_mock,patch_users_response_success_mock):
    """Test to update specific info of user with success"""
    #Arrange
    user_id = "1"
    payload = patch_user_patch_dto_mock.model_dump(mode='json')
    print(f"test_patch_success - payload ======> {payload}")
    mock_response = patch_users_response_success_mock.model_dump(mode='json')
    status_code = 200
    #Act
    response = test_client.patch("/v1/users/"+user_id, json=payload)
    response_filtered = response.json()
    del response_filtered['data']['updated_at']
    del mock_response['data']['updated_at']
    #Assert
...
def test_patch_user_error_un_processable_content(test_client,
                                                 patch_user_patch_dto_mock,general_error):
...
def test_delete_user_success_empty_register(test_client,delete_success):
    ...
    status_code = 200
    #Act
    response = test_client.delete("/v1/users/"+user_id)
    #Assert
    assert response.status_code == status_code
    assert response.json() == data
