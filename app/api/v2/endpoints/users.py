...
@router.get(
    ...
    response_model=UsersAndPokemonsResponseList,
...
async def get_users(db: Session = Depends(get_db), client=Depends(get_http_client)):
    ...
    try:
        ...
        return UsersAndPokemonsResponseList(
            code=status.HTTP_200_OK, message="Data obtained", data=users
        )
...
@router.get(
    ...
    response_model=UserAndPokemonsInfoResponse,
    status_code=status.HTTP_200_OK,
    dependencies=[Depends(get_db)],
)
async def get_users_by_id(
    id: int, db: Session = Depends(get_db), client=Depends(get_http_client)
):
    """Get user info"""
    user_service = UserAndPokemonService(db_session=db, http_client=client)
    ...
    try:
        ...
        response = UserAndPokemonsInfoResponse(
            code=status.HTTP_200_OK, message="Data obtained", data=user
        )
        return response
...
@router.post(
    "/users",
    summary="Create user",
    response_model=UsersAndPokemonsResponse,
    status_code=status.HTTP_201_CREATED,
    dependencies=[Depends(get_db)],
)
async def add_user(
    user_data: UserAndPokemonsCreateDTO,
    ...
    try:
        ...
        response = UsersAndPokemonsResponse(
            code=status.HTTP_201_CREATED, message="User created", data=user
        )
        return response
...
async def update_user(
    ...
    try:
        ...
        response = UsersAndPokemonsResponse(
            code=status.HTTP_200_OK, message="User info updated", data=user
        )
        return response
    except ValueError as e:
        raise HTTPException(
...
async def update_user(
    id: int,
    user_data: UserAndPokemonsPatchDTO,
    db: Session = Depends(get_db),
    client=Depends(get_http_client),
):
