import pytest
from app.schemas.user_and_pokemon_dto import UsersAndPokemonsDTO, UserAndPokemonsDTO, UserAndPokemonsCreateDTO, UserAndPokemonsUpdateDTO, UserAndPokemonsPatchDTO, UsersAndPokemonsResponse, UserAndPokemonsInfoResponse, UsersAndPokemonsResponseList
...
@pytest.fixture
def get_all_users_and_pokemons_response_success_mock(users_and_pokemon_dto_mock: UsersAndPokemonsDTO):
...
# Response consolidated
@pytest.fixture
def get_user_and_pokemons_info_by_id_response_success_mock(user_and_pokemon_dto_mock: UserAndPokemonsDTO):
    return UserAndPokemonsInfoResponse(
        code=200,
        message="Data obtained",
        data=user_and_pokemon_dto_mock
    )
...
# Response:
@pytest.fixture
def post_users_and_pokemons_response_success_mock(users_and_pokemon_dto_mock: UsersAndPokemonsDTO):
    return UsersAndPokemonsResponse(
        code=201,
        message="User created",
        data=users_and_pokemon_dto_mock
    )
@pytest.fixture
def post_error():
...
# Response:
@pytest.fixture
def put_users_and_pokemons_response_success_mock(put_users_and_pokemon_dto_mock: UsersAndPokemonsDTO):
    return UsersAndPokemonsResponse(
        code=200,
...
# Response:
@pytest.fixture
def patch_users_and_pokemons_response_success_mock(patch_users_and_pokemon_dto_mock: UsersAndPokemonsDTO):
    return UsersAndPokemonsResponse(
        code=200,
        message="User info updated",
