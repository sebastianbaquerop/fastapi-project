...
class UserAndPokemonRepository:
    ...
    def get_by_email(self, email: str) -> Optional[UsersAndPokemons]:
        return (
            self.db.query(UsersAndPokemons)
            .filter(UsersAndPokemons.email == email)
            .first()
        )
    ...
    def get_by_id(self, user_id: str) -> Optional[UsersAndPokemons]:
        return (
            self.db.query(UsersAndPokemons)
            .filter(UsersAndPokemons.id == user_id)
            .first()
        )

    def update(
        self, user_id: int, user_data: UserAndPokemonsCreateDTO
    ) -> Optional[UsersAndPokemons]:
    ...
    def patch(
        self, user_id: int, user_data: UserAndPokemonsPatchDTO
    ) -> Optional[UsersAndPokemons]:
        user = self.get_by_id(user_id)
        if not user:
            return None
